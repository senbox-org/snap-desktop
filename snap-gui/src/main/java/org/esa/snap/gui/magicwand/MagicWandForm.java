package org.esa.snap.gui.magicwand;
...
class MagicWandForm {
    ...
    public JPanel createPanel() {
        ...
        AbstractButton filterButton = createButton(ImageUtilities.loadImageIcon("org/esa/snap/gui/icons/Filter24.gif", false));
    ...
    void showBandChooser() {
        ...
        if (view == null) {
            SnapApp.getInstance().showInfoDialog("Please select an image view first.", null);
            return;
        }
        ...
        if (bands.length == 0) {
            SnapApp.getInstance().showInfoDialog("No bands in product.", null);
            return;
        }

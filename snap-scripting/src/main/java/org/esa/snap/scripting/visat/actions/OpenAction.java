...
public class OpenAction extends ScriptConsoleAction {
    ...
    public OpenAction(ScriptConsoleTopComponent scriptConsoleTC) {
        super(scriptConsoleTC, "Open...", ID, TangoIcons.actions_document_open(TangoIcons.Res.R16));
    }

    @Override
    public void actionPerformed(ActionEvent e) {
        JFileChooser fs = OpenAction.createFileChooser(getScriptManager().getEngineFactories());
        int i = fs.showOpenDialog(getScriptConsoleTopComponent());
        if (i == JFileChooser.APPROVE_OPTION) {
            getScriptConsoleTopComponent().openScript(fs.getSelectedFile());
        }
    }

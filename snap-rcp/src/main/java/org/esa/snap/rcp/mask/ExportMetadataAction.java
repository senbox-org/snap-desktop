...
@ActionID(
        category = "File",
        id = "org.esa.snap.rcp.mask.ExportMetadataAction"
)
@ActionRegistration(
        displayName = "#CTL_ExportMetadataAction_MenuText",
        popupText = "#CTL_ExportMetadataAction_MenuText"
)
@ActionReferences(
        {
                @ActionReference(
...
@NbBundle.Messages({
        ...
        "CTL_ExportMetadataAction_ShortDescription=Export Product Metadata..."
})
public class ExportMetadataAction extends AbstractAction implements LookupListener, HelpCtx.Provider {
    ...
    private static final String HELP_ID = "exportMetadata";
...
//            return;
//        }
//        final RasterDataNode rasterDataNode = view.getRaster();
//        final String msgText = "How do you want to export the metadata?\n" +
//                "Element'" + view.getName() + "'will be exported.\n"; /*I18N*/
...
//            final File file = promptForFile(createDefaultFileName(rasterDataNode));
//            if (file == null) {
//                return; // Cancel
//            }
...
//        swingWorker.execute();
    ...
    private static String createDefaultFileName(final RasterDataNode rasterDataNode) {
        return FileUtils.getFilenameWithoutExtension(rasterDataNode.getProduct().getName()) +
                "_" +
    ...
    private static File promptForFile(String defaultFileName) {
        ...
        File file = null;
        while (file == null) {
            file = SnapDialogs.requestFileForSave(DLG_TITLE, false, null, ".txt", defaultFileName, null, "exportMetadata.lastDir");
            if (file == null) {
                return null; // Cancel
            } else if (file.exists()) {
                int status = JOptionPane.showConfirmDialog(SnapApp.getDefault().getMainFrame(),
                        "The file '" + file + "' already exists.\n" + /*I18N*/
                                "Overwrite it?",
                        MessageFormat.format("{0} - {1}", SnapApp.getDefault().getAppName(), DLG_TITLE),
...
//        ProductNodeView view = SnapApp.getDefault().getSelectedProductSceneView();
//        setEnabled(view instanceof ProductNodeView);
    }
...
//        private final ProductSceneView metadataTable;

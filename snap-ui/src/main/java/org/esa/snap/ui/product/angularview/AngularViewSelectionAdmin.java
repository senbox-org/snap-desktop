...
class AngularViewSelectionAdmin {
    ...
    void evaluateAngularViewSelections(DisplayableAngularview angularView) {
        List<Boolean> selected = new ArrayList<Boolean>();
        int numberOfSelectedBands = 0;
        for (int i = 0; i < angularView.getAngularBands().length; i++) {
            final boolean bandSelected = angularView.isBandSelected(i);
    ...
    boolean isAngularViewSelected(int row) {
    ...
    void updateAngularViewSelectionState(int row, int newState) {

...
public class CopyActionExt implements ProductLibraryActionExt {
    ...
    public JButton getButton(final JPanel panel) {
        if (button == null) {
            button = DialogUtils.createButton("copySelectedButton", "Copy to clipboard", copyIcon, panel, DialogUtils.ButtonStyle.Icon);
        }
        return button;
    }
    ...
    public void performAction(final ProgressMonitor pm) {
        final File[] fileList = actionHandler.getToolView().getSelectedFiles();
